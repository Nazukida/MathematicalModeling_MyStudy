...
4. [Task 3: AHP-TOPSIS双阶评价体系](#4-task-3-ahp-topsis双阶评价体系)
5. [Task 4: 全球教育战略建模框架](#5-task-4-全球教育战略建模框架)
6. [数据流向与技术实现](#6-数据流向与技术实现)
7. [附录：模型流程图](#7-附录模型流程图)
...
### 1.1 研究背景与逻辑梳理
本模型框架旨在解决高等教育在人工智能变革时代面临的核心挑战：**“预测难、决策难、评价难、推广难”**。我们构建了一套闭环系统，通过定量预测未来需求，指导微观教育资源分配，并建立科学评价体系，最终生成可全球推广的战略框架。

### 1.2 核心任务逻辑
整个建模过程分为层层递进的四个阶段：
*   **预测阶段 (Task 1)**：回答“未来需要什么样的人才？”——利用灰色预测与S曲线量化AI对不同职业的替代与增强效应。
*   **决策阶段 (Task 2)**：回答“大学该如何调整？”——在有限资源和多重红线约束下，寻找招生规模与课程设置的最优解。
*   **评价阶段 (Task 3)**：回答“改得好不好？”——引入环境、公平、安全等多维指标，对比不同策略的综合价值。
*   **推广阶段 (Task 4)**：回答“其他学校怎么办？”——构建三维战略空间，通过聚类分析为全球不同类型的大学提供定制化建议。
...
1.  **基准趋势预测 (Baseline Prediction)**
    *   利用 **GM(1,1) 灰色预测模型**，基于BLS（美国劳工统计局）的历史就业数据，计算假设没有AI介入时的“自然增长趋势”。这解决了就业数据样本少、波动大的问题。
...
2.  **微观层：课程结构优化 (Curriculum Optimization)**
    ...
    *   **引入“三大红线”硬约束**：
        ...
        2.  **环境约束**：通过“绿色封顶 (Green Cap)”，严格限制高能耗AI训练课程的学分占比。
...
3.  **保障层：职业路径弹性 (Career Elasticity)**
    ...
    *   确保即使目标职业消失，学生也能凭借高通用的技能组合平滑转型，作为模型的“安全网”。

---

## 4. Task 3: AHP-TOPSIS双阶评价体系
**核心目标**：跳出单一的“就业率”视角，多维度评价教育策略的优劣。

### 工作流程详解：

1.  **第一阶段：构建准则体系 (Criteria Weighting)**
    *   利用 **AHP (层次分析法)** 确定评价维度的权重。
    *   不仅仅考虑 **C1 就业竞争力**，还重点引入 **C2 环境友好度** (主要针对大模型能耗)、**C3 AI安全与伦理**、**C4 教育公平性**。这反映了从“市场导向”向“可持续发展导向”的价值观转变。

2.  **第二阶段：方案综合排序 (Strategy Ranking)**
    *   对比 **Strategy A (传统市场驱动模型)** 和 **Strategy B (红线约束模型)**。
    *   使用 **TOPSIS (逼近理想解排序法)** 计算两种策略与“完美理想解”的欧氏距离。
    *   **结论分析**：虽然Strategy A在就业指标上略高，但Strategy B在安全、环境公平性上具有压倒性优势，因此综合得分更高，证明了引入约束机制的必要性。
...
*   **Task 2 $\rightarrow$ Task 3**：优化后的课程方案 ($X^*$) 成为了Task 3评价体系的被评价对象。
*   **Task 1/2/3 $\rightarrow$ Task 4**：前三个模型提取的特征参数 ($D1, \lambda, \gamma$) 共同构成了Task 4的三维坐标轴。
...
### 7.1 主工作流程图 (Main Workflow)
涵盖了从数据输入到最终全球策略输出的全过程，清晰展示了四个Task之间的逻辑连接。

![Model Workflow Diagram](figures/model_workflow_diagram.png)

### 7.2 详细任务流程图 (Detailed Task Flow)
